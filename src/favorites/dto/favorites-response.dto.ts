...
export const ApplicationFavoritesResponseSchema = z
  .object({
    applicationId: z.number().int().positive().openapi({ example: 1 }),
...
export const ApplicationFavoritesCountResponseSchema = z
  .object({
    applicationId: z.number().int().positive().openapi({ example: 1 }),
...
export type ApplicationFavoritesCountResponse = z.infer<
  typeof ApplicationFavoritesCountResponseSchema
>;
